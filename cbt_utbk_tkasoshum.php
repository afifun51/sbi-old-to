<?php
	...
	$temp = mysql_query("SELECT * FROM soal WHERE code = 'TS' and number = '$qNumberAfterRandom' ");
	$myArray = mysql_fetch_array($temp);
	...
	if (isset($_POST['prev'])) {  
		...
		header('location: cbt_utbk_tkasoshum.php');
		exit;
	}
	...
	if (isset($_POST['selesai'])) { //Nanti diisi jumlah soal max
		...
		header('location: cbt_utbk_tkasoshum_result.php');		
		exit;			
	...
	function has_taken_tryout($username) {
...
history.pushState(null, null, 'cbt_utbk_tkasoshum.php');
...
//var seconds = 8;
	setInterval(
        function(){
          if (seconds <= 0) {
			window.location = 'cbt_utbk_tkasoshum_result.php';
          }
          else {
			seconds = seconds -1;
                    ...
                    <div class="col-xl-8 mb-3 mb-xl-5">
                        ...
                        <div class="card mb-3 mb-md-4">
                            ...
                            <div class="card-header d-flex border-bottom align-items-center">
                                <h5 class="font-weight-semi-bold mb-0"><span class="badge badge-blue mb-1 mr-2">Tes Kemampuan Akademik (TKA) Soshum</span></h5>
                            </div>
                            <div class="card-body">
					...
					for ($r = 0; $r < $numberOfLine ; $r++) {
						for ($c = 1; $c <= 10 && $count < $numberOfQuestion ; $c++) {							
							...
							if($mark == 0) {
								...
								//echo $qAnswer[($r*5)+$c];
								//echo "</td>";								
								echo '<a href="cbt_utbk_tkasoshum.php?qNum='.$num.'" class="badge badge-lg bg-soft-primary badge-question-status position-relative font-weight-semi-bold text-primary p-0 mb-3 mr-5">';
